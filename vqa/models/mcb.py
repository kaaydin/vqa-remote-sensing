...
class CompactBilinearPoolingFn(Function):
    ...
    def forward(ctx, h1, s1, h2, s2, output_size, x, y, force_cpu_scatter_add=False):
        ...
        re = torch.fft.irfft(torch.stack((re_prod, im_prod), -1), n=output_size, dim=-1)
        re = re.real  # Ensures the output is purely real
        return re
    ...
    def backward(ctx,grad_output):
        ...
        grad_fx = torch.fft.irfft(torch.stack((grad_re_fx, grad_im_fx), -1), n=ctx.output_size, dim=-1)
        ...
        grad_fy = torch.fft.ifft(torch.stack((grad_re_fy, grad_im_fy), -1), n=ctx.output_size, dim=-1)
